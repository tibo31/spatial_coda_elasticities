map_elas_bar <- function(spatial_direct, spatial_indirect, spatial_total, coords_s,
  ...
  if (x.legend != "none") {
    text(coords_s[k, 1] - shift_coords_x, 
         coords_s[k, 2] - max_bar/2, "direct", srt = 45, cex = cex.legend)
    text(coords_s[k, 1], 
         coords_s[k, 2]  - max_bar/2, "indirect", srt = 45, cex = cex.legend)
    text(coords_s[k, 1] + shift_coords_x, 
         coords_s[k, 2] - max_bar/2, "total", srt = 45, cex = cex.legend)
  ...
  legend(x.legend, legend = names_D, fill = q4, cex = 1, box.lwd = -4)
}
